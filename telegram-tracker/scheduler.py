...
from poller import send_morning_poll, send_evening_poll, send_text_message, send_countdown_alerts
...
async def _morning_with_countdowns(context) -> None:
    """Sends countdown alerts first, then the morning checklist."""
    await send_countdown_alerts(context)
    await send_morning_poll(context)
...
def setup_jobs(app: Application) -> None:
    ...
    jq = app.job_queue
    jq.run_daily(_morning_with_countdowns, time(m_hour, m_min, tzinfo=tz), name="morning_poll")
